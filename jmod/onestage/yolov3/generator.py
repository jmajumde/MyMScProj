...
from MyMScProj.jmod.onestage.image import correct_bounding_boxes, random_distort_image, random_flip, \
...
class BatchGenerator(Sequence):
    ...
    def __init__(self,
        ...
        self.jitter = jitter
        self.norm = norm
        self.anchors = [BoundingBox(0, 0, anchors[2 * i], anchors[2 * i + 1]) for i in range(len(anchors) // 2)]
        self.net_h = 416
        self.net_w = 416
    ...
    def __getitem__(self, idx):
        ...
        for train_instance in self.instances[l_bound:r_bound]:
            ...
            #print("image net h,w", (net_h, net_w))
            #imageReader = ImageEncoder(train_instance['filename'], net_h, net_w)
            #img, all_objs = imageReader.fit(train_instance)
            ...
            for obj in all_objs:
                if obj['xmax'] > obj['xmin'] and obj['ymax'] > obj['ymin'] and obj['name'] in self.labels:
                    ...
                    for i in range(len(self.anchors)):
                        ...
                        iou = AnchorBox().bbox_iou(shifted_box, anchor)
